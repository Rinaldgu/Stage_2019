...
for j=1:size(BM,2)
    if BM(10,j)>70 &  BM(10,j)<83.5
        if BM(9,j)>180 & BM(9,j)<240
            if BM(3,j)==1
                if BM(6,j)>0
                   e1c(j)=BM(6,j); %Salinité
                   x1c(j)=BM(3,j)+(BM(2,j)*(100/31))/100;  %On crée un vecteur qui servira d'abcisse pour le graphique. Chaque valeurs de mld du mois de janvier a une valeur en abcisse correspondant entre 1 et 2 selon le jour de la mesure.
                   s1=s1+BM(6,j);% Somme
                   n1=n1+1; % Nombre de valeurs
                   janvier_canada=s1/n1; %Moyenne
                end
            end
        end
    end
